...
vehicleTable <- table(tolower(ny_df$vehicle_type_code1))
vehicleTable <- vehicleTable[order(vehicleTable)]
tail(vehicleTable, 10)
factorTable <- table(tolower(ny_df$contributing_factor_vehicle_1))
factorTable <- factorTable[order(factorTable)]
tail(factorTable, 10)
#Need to check with Tom on how to not include the "Unspecified" category before using the tail() function to cut down the factorTable; same thing with "unknown" and "other" for the vehicleTable
```

# Include horizontal bar graph on the vehicle Table and factor Table - will need to put into dataframe format to do this in ggplot - good segue into picking the top 5 or 10 categories
# May not need to use coodflip for this as ggplot now accepts categorical variables on the y-axis
# maybe use the Cleveland dot plot for this instead of a bar graph:
library(ggplot2)
...
mpg %>% 
  ...
  arrange(cty) %>% 
  mutate(trans = factor(trans, levels = unique(trans))) %>% 
  ggplot(aes(x = cty, y = trans)) +
  geom_point()
...
# need to made a date out of the month and year and then graph it including facets by borough